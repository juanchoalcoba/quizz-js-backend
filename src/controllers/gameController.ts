...
export const getUserSessions = async (req: Request & { user?: any }, res: Response) => {
  try {
    const sessions = await GameSession.find({ userId: req.user.id }).sort({ createdAt: -1 });
    res.json(sessions);
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Error obteniendo partidas del usuario' });
  }
};
